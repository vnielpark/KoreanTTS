...
from preprocess import path_replace
...
def prepare_align(in_dir, meta):
    ...
    with open(meta_path, encoding='utf-8') as f:
        for line in f:
            ...
            basename_path = path_replace(os.path.join(in_dir,'wavs',basename), '/', '\\')
            with open(basename_path,'w') as f1:
                f1.write(text)
...
def build_from_path(in_dir, out_dir, meta):
    ...
    for idx, param_name in enumerate(param_name_list):
        np_path = path_replace(os.path.join(out_dir, param_name), '/', '\\')
        np.save(np_path, param_list[idx])
...
def process_utterance(in_dir, out_dir, basename, scalers):
    ...
    wav_bak_path = path_replace(wav_bak_path, '/', '\\')
    wav_path = os.path.join(in_dir, 'wavs', '{}.wav'.format(basename))
    wav_path = path_replace(wav_path, '/', '\\')

    tg_path = os.path.join(out_dir, 'TextGrid', '{}.TextGrid'.format(basename)) 
    tg_path = path_replace(tg_path, '/', '\\')
    if not os.path.exists(tg_path):
        tg_path = os.path.join(out_dir, 'TextGrid', '{}.TextGrid'.format(wav_bak_basename)) 
        tg_path = path_replace(tg_path, '/', '\\')
        if not os.path.exists(tg_path):
            return None
    ...
    ali_path = path_replace(os.path.join(out_dir, 'alignment', ali_filename), '/', '\\')
    np.save(ali_path, duration, allow_pickle=False)

    # Save fundamental prequency
    f0_filename = '{}-f0-{}.npy'.format(hp.dataset, basename)
    f0_path = path_replace(os.path.join(out_dir, 'f0', f0_filename), '/', '\\')
    np.save(f0_path, f0, allow_pickle=False)

    # Save energy
    energy_filename = '{}-energy-{}.npy'.format(hp.dataset, basename)
    energy_path = path_replace(os.path.join(out_dir, 'energy', energy_filename), '/', '\\')
    np.save(energy_path, energy, allow_pickle=False)

    # Save spectrogram
    mel_filename = '{}-mel-{}.npy'.format(hp.dataset, basename)
    mel_path = path_replace(os.path.join(out_dir, 'mel', mel_filename), '/', '\\')
